...
<table>
  <tr>
    ...
    <th class="c6">END TIME</th>
    <th class="c4">AVAILABILITY</th>
  </tr>
	<c:forEach var="tempSchedule" items="${schedule}" varStatus="status" >
  <tr>
    <td class="c1">${tempSchedule.startTime}</td>
    <td class="c2">${tempSchedule.endTime}</td>
		<c:choose>
						<c:when test="${tempSchedule.flag}">
  						  <td class="c3">Booked</td>
							</c:when>
					<c:otherwise>
						<td class="c3"><a href="appointmentBooking?startTime= ${tempSchedule.startTime}&endTime=${tempSchedule.endTime}"><input class="book" type="button" name="book" value="Book"></a></td>
					</c:otherwise>
		</c:choose>
	</tr>
